...
class ScriptArguments:
    # 学习注释：这一组不是 Transformers 通用训练参数，而是 MedicalGPT 自己定义的
    # PEFT/LoRA、序列长度、对话模板和工具调用数据参数。
...
def main():
    ...
    # 学习注释：在模型初始化和数据 shuffle 之前固定随机种子，才有机会复现实验。
    ...
    # 学习注释：IGNORE_INDEX 通常是 -100。CrossEntropyLoss 遇到该标签会跳过对应 token，
    # 后面正是靠它实现“看得到用户输入，但不对用户输入计算语言模型 loss”。
    IGNORE_INDEX = LabelSmoother.ignore_index if data_args.ignore_pad_token_for_loss else tokenizer.pad_token_id

    # 学习注释：数据有两条入口：`dataset_name` 从 Hub 加载；否则递归读取本地目录中的
    # JSONL。若没有独立 validation，代码会固定 seed=42 从 train 中切出一部分。
    ...
    def preprocess_function(examples):
        ...
        学习注释：这是 SFT 最关键的数据路径。批量输入的 conversations 先被整理为
        user/assistant 轮次，再经 chat template 拼成文本，最后输出 Trainer 需要的
        input_ids、attention_mask 和 labels。不要把它当成普通的“tokenize 一下”。
        """
        input_ids_list = []
        attention_mask_list = []
        targets_list = []
        ...
        def get_dialog(examples):
            # 学习注释：每个 yield 的 dialog 按 [用户侧文本, 助手答案, ...] 交替排列。
            # function_call/observation 也会先转换为目标模型对应的工具调用文本格式；
            # 这只是监督学习已有轨迹，并不代表模型在训练中真的执行了工具。
        ...
        for dialog in get_dialog(examples):
            ...
            for i in range(len(dialog) // 2):
                # 学习注释：source_ids 是 system/user/生成提示，target_ids 是助手答案。
                # 多轮对话会依次追加，并在每轮答案末尾补 EOS；超长时按比例截断。
                ...
                if script_args.train_on_inputs:
                    # 学习注释：True 时用户侧和助手侧 token 都参与 next-token loss。
                    labels += source_ids + target_ids + [tokenizer.eos_token_id]
                else:
                    # 学习注释：默认 False。输入仍保留在 input_ids 中供模型作为条件读取，
                    # 但 source_ids 对应的 label 被设为 -100，只学习助手答案 target_ids。
    ...
    if model_args.model_name_or_path:
        ...
        # 学习注释：`load_in_4bit` 只表示量化加载；只有同时打开 `qlora`，这里才使用
        # QLoRA 常见的 NF4 + double quant 配置。量化的是冻结的基础模型，不是把 LoRA
        # 可训练参数也压成 4 bit。
    ...
    if script_args.use_peft:
        # 学习注释：PEFT/LoRA 冻结绝大多数基础模型权重，只在选中的线性层旁增加低秩
        # A/B 矩阵。`target_modules=all` 会由 find_all_linear_names 自动寻找这些线性层。
        ...
        if script_args.peft_path is not None:
            ...
            if load_in_8bit or load_in_4bit:
                # 学习注释：量化模型在注入 LoRA 前需要做 k-bit 训练准备，例如处理部分
                # dtype、梯度和 gradient checkpointing 兼容性。
    ...
    # 学习注释：collator 在“组 batch”时动态 padding；input_ids 用 tokenizer 的 pad token，
    # labels 用 IGNORE_INDEX padding，避免补齐位置产生 loss。
    ...
    # Initialize our Trainer
    # 学习注释：Trainer 接管 dataloader、前向/反向、梯度累积、评估和 checkpoint。
    # SavePeftModelTrainer 覆盖了保存逻辑，使 PEFT 训练主要保存 adapter 而非复制完整底模。
    ...
    if training_args.do_train:
        if trainer.is_world_process_zero():
            ...
            replaced_labels = [label if label != IGNORE_INDEX else tokenizer.pad_token_id for label in
                               sample['labels'][0]]
            logger.debug(f"Decode labels[0]:\n{tokenizer.decode(replaced_labels)}")
        # 学习注释：resume_from_checkpoint 恢复的不只是权重，还包括优化器、scheduler
        # 和训练步数；这与“加载一个已有 LoRA 后重新开始训练”是两件事。
        checkpoint = None
        ...
        if trainer.is_world_process_zero():
            ...
            # 学习注释：ZeRO-3 下参数被分片，保存前必须聚合；普通 LoRA/单卡走常规保存。
            if is_deepspeed_zero3_enabled():
    ...
    if training_args.do_eval:
        ...
        # 学习注释：perplexity = exp(eval_loss)，只衡量当前 label token 上的语言模型拟合；
        # 它不能替代医学问答准确率、格式遵循或幻觉评测。
