...
def evaluate_hybrid_run(
    ...
    *,
    ...
    gold_ranks = {
        query_id: first_gold_rank(fused_rankings[query_id], qrels[query_id])
...
def evaluate_hybrid_document_run(
    dataset_dir: Path,
    bm25_rankings_path: Path,
    dense_rankings_path: Path,
    output_dir: Path,
    *,
    ...
    """Fuse two document-level rankings (BM25-doc + Dense-doc) with RRF and
    evaluate without any chunk collapse (P0-5/P0-7).
    """
    from medical_graphrag.evaluation.document import _validate_document_run_context
    ...
    bm25_rows = read_jsonl(bm25_rankings_path)
    dense_rows = read_jsonl(dense_rankings_path)
    for rows, label in ((bm25_rows, "bm25"), (dense_rows, "dense")):
        if len(rows) != len(questions) or {str(r["query_id"]) for r in rows} != set(
            questions
        ):
            raise ValueError(f"{label} ranking query set does not match questions")
        _validate_document_run_context(
            run_context[label],
            rows,
            bm25_rankings_path if label == "bm25" else dense_rankings_path,
            dataset_manifest_sha256,
            dataset_manifest["artifact_hashes"],
        )
    ...
    bm25_by_qid = {str(r["query_id"]): [str(h["doc_id"]) for h in r["hits"]] for r in bm25_rows}
    dense_by_qid = {str(r["query_id"]): [str(h["doc_id"]) for h in r["hits"]] for r in dense_rows}
    fused_rankings: dict[str, list[str]] = {}
    for query_id, row in questions.items():
        fused = fuse_rrf(bm25_by_qid[query_id], dense_by_qid[query_id], k=k)
        if not fused:
            raise ValueError(f"empty fused ranking for {query_id}")
        fused_rankings[query_id] = fused
    ...
    metrics: dict[str, Any] = {}
    for split in ("dev", "test"):
        ids = [qid for qid, q in questions.items() if q["split"] == split]
        if not ids:
            continue
        split_qrels = {qid: qrels[qid] for qid in ids}
        split_rankings = {qid: fused_rankings[qid] for qid in ids}
        metrics[split] = {
            "sample_count": len(ids),
            **evaluate_rankings(split_qrels, split_rankings, ks=(1, 5, 10)),
        }

    test_ids = [qid for qid, q in questions.items() if q["split"] == "test"]
    gold_ranks = {
        qid: first_gold_rank(fused_rankings[qid], qrels[qid]) for qid in test_ids
    }
    success_all = [qid for qid in test_ids if gold_ranks[qid] is not None and gold_ranks[qid] <= 10]
    failure_all = [qid for qid in test_ids if gold_ranks[qid] is None or gold_ranks[qid] > 10]
    cases: dict[str, Any] = {}
    for label, ids in (("success", success_all[:5]), ("failure", failure_all[:5])):
        cases[label] = [
            {
                ...
                "gold_title": documents[qrels[qid][0]]["title"],
                "gold_rank": gold_ranks[qid],
                "top_documents": [
                    {"doc_id": doc_id, "title": documents[doc_id]["title"], "fused_rank": rank}
                    for rank, doc_id in enumerate(fused_rankings[qid][:10], start=1)
                ],
            }
            for qid in ids
        ]
