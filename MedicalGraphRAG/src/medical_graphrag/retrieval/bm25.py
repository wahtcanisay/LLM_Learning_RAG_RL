...
def export_document_collection(dataset_dir: Path, output_dir: Path) -> dict[str, Any]:
    """Export full documents as a Lucene collection (document retrieval unit).

    Each document becomes one Lucene doc with ``id = doc_id`` and
    ``contents = full document.content`` (untruncated). The report binds to
    ``documents.jsonl`` so the index/search/evaluate chain audits the real
    input for the document-level baselines.
    """
    manifest = validate_frozen_dataset(dataset_dir)
    documents = _read_jsonl(dataset_dir / "documents.jsonl")
