...
| scifact MRR | 0.630 | 运行中 | 运行中 | 运行中 | 运行中 |

### 4.4 核心发现

1. **图检索在短摘要上弱**:LinearRAG 同款 Graph 在 pubmedqa(短摘要)明显输给 Dense/Hybrid(0.768 vs 0.959);在 nfcorpus 接近中性(0.480 vs 0.506)。
2. **软边效果因数据而异**:pubmedqa(单 gold + 主题相近干扰)上软边**有害**(Graph-Sim 0.756 < Graph-EP 0.768);nfcorpus(多相关)上**中性**(0.477 ≈ 0.480)。
3. **Hybrid/Hybrid2 始终最优**:词项+语义互补;reranker 在多跳(hotpotqa)增益最大。
4. **诚实负结果**:图检索价值在现有数据上未体现——这是方法对数据类型的固有结果,不是实现缺陷。
